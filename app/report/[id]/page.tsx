        ...
        <div style={{ padding: "24px", flex: 1, display: "flex", flexDirection: "column", gap: 20 }}>
          {readonly ? (
            <>
              ...
              {annotation.hypothesis && <div><label style={LBL}>Hypothesis / user story</label><p style={{ ...field, border: `1px solid ${B.border}` }}>{annotation.hypothesis}</p></div>}
            </>
          ) : (
            <>
              ...
              <div>
                <label style={LBL}>Hypothesis / user story</label>
                <textarea value={form.hypothesis} disabled placeholder="Auto-generated by AI — not yet available for manual findings" rows={3}
                  style={{ ...field, resize: "vertical", background: "#f0f0f0", color: B.muted, cursor: "not-allowed" }} />
              </div>
            </>
          )}
...
function AnnotatedScreenshot({
  ...
  function handleContainerClick(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const newAnnotation = { id: genId(), number: page.annotations.length + 1, x, y, category: "UX", severity: "High", title: "", detail: "", recommendation: "", hypothesis: "", source: "manual" };
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: 18 }}>
        {readonly ? (
          ...
          <>
            ...
            {annotation.hypothesis && <div><label style={LBL}>Hypothesis / user story</label><p style={fieldBlock}>{annotation.hypothesis}</p></div>}
          </>
        ) : (
          // ── Editable form ──
          <>
